...
// Modified Scott Jin to add a column to show url and a matching cloudfront access
...
const s3ExplorerColumns = {
    check: 0, object: 1, folder: 2, date: 3, timestamp: 4, storageclass: 5, size: 6, durl: 7
};
...
function ViewController($scope, SharedService) {
    ...
    $scope.renderDownload = (data, _type, full) => {
        if ($scope.view.settings.cloudfront != '') {
            if (isfolder(data.Key)) return '';
            else if (data.Key == '') return '';
            else {
                const link = $scope.view.settings.cloudfront + '/' + data.Key.replaceAll('"', "");
                return '<a href="' + link + '"data-s3="outer">' + link + '</a>';
            }
        }
        return data.Key;
    }
    ...
    this.Downloadrender = (data, type, full) => $scope.renderDownload(data, type, full);
    ...
    $tb.DataTable({
        ...
        aoColumnDefs: [
            ...
            {
                aTargets: [7], mData: this.Downloadrender,
            },
        ],
    });
...
function SettingsController($scope, SharedService) {
    ...
    $scope.settings = {
        auth: 'anon', region: '', bucket: '', entered_bucket: '', selected_bucket: '', view: 'folder', delimiter: '/', prefix: '', buckets: [], cloudfront: '',
    };
    ...
    presetbuckets.forEach((element) => { $scope.settings.buckets.push(element.name); });
    if ($.inArray($scope.settings.buckets, $scope.settings.extra) === -1) {
        $scope.settings.buckets.push($scope.settings.extra);
    ...
    $scope.update = () => {
        ...
        if ($scope.settings.entered_bucket) {
            if (!$scope.settings.extra) {
                $scope.settings.extra = [];
            }
            if ($.inArray($scope.settings.entered_bucket, $scope.settings.extra) === -1) {
                $scope.settings.extra.push($scope.settings.entered_bucket);
                $scope.settings.extra = $scope.settings.extra.sort();
            }
        }
        ...
        for (let i = 0; i < presetbuckets.length; i++) {
            if (presetbuckets[i].name == $scope.settings.bucket) {
                $scope.settings.cloudfront = presetbuckets[i].cloudfront;
                break;
            }
        }
        SharedService.changeSettings($scope.settings);
